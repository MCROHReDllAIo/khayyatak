"use client";

import { useEffect, useMemo, useState } from "react";
import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip } from "recharts";
import { useLocale } from "@/lib/context/locale-context";
import type { CityCoverage } from "@/lib/admin/types";
import { OmanMapIntel } from "@/components/national/OmanMapIntel";

type AnalyticsPayload = {
  coverage: CityCoverage[];
  customers: { total: number };
  operations: { delivered: number };
  marketplace: { avgOrder: number; repeatRate: number };
  topTailors: Array<{ id: string }>;
};

export default function NationalIntelligencePage() {
  const { t } = useLocale();
  const [data, setData] = useState<AnalyticsPayload | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/admin/analytics?range=30d")
      .then((r) => r.json())
      .then((json) => setData(json))
      .catch(() => setData(null))
      .finally(() => setLoading(false));
  }, []);

  const stats = useMemo(() => {
    const coverage = data?.coverage ?? [];
    const totalOrders = coverage.reduce((s, c) => s + c.orders, 0);
    const totalGmv = coverage.reduce((s, c) => s + c.gmv, 0);
    const avgOrder = totalOrders > 0 ? Math.round((totalGmv / totalOrders) * 100) / 100 : 0;
    return {
      avgOrderPrice: avgOrder,
      monthlyOrders: totalOrders,
      activeTailors: data?.topTailors?.length ?? coverage.reduce((s, c) => s + c.tailors, 0),
      growthPercent: data?.marketplace?.repeatRate ?? 0,
      ordersByCity: coverage.map((c) => ({
        city: c.name_ar,
        orders: c.orders,
      })),
    };
  }, [data]);

  return (
    <div className="space-y-12 md:space-y-16 max-w-5xl">
      <header>
        <p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-amber-700 mb-2">
          {t("ذكاء السوق الوطني", "National Market Intelligence")}
        </p>
        <h1 className="editorial-title">{t("الذكاء الوطني للخياطة", "National Tailoring Intelligence")}</h1>
        <p className="editorial-sub mt-4">
          {t(
            "فهم مجمّع للسوق المحلي — دون كشف بيانات الأفراد.",
            "Aggregate understanding of the local market — no personal data exposed."
          )}
        </p>
      </header>

      {loading ? (
        <div className="rounded-3xl border bg-white p-12 text-center text-muted-foreground animate-pulse">
          {t("جاري تحميل البيانات...", "Loading data...")}
        </div>
      ) : (
        <OmanMapIntel cities={data?.coverage ?? []} />
      )}

      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 py-4">
        {[
          { label: t("متوسط الطلب", "Avg order"), value: stats.avgOrderPrice > 0 ? `${stats.avgOrderPrice} ر.ع` : "—" },
          { label: t("إجمالي الطلبات", "Total orders"), value: String(stats.monthlyOrders) },
          { label: t("خياطون نشطون", "Active tailors"), value: String(stats.activeTailors) },
          { label: t("معدل إعادة الطلب", "Repeat rate"), value: stats.growthPercent > 0 ? `${stats.growthPercent}%` : "—" },
        ].map((stat) => (
          <div key={stat.label} className="border-s-2 border-primary/30 ps-4">
            <p className="text-xs uppercase tracking-wider text-muted-foreground">{stat.label}</p>
            <p className="text-3xl font-bold text-navy mt-1 tabular-nums">{stat.value}</p>
          </div>
        ))}
      </div>

      <section className="fashion-divider pt-12">
        <h2 className="text-xl font-bold text-navy mb-6">{t("الطلب حسب المدينة", "Demand by city")}</h2>
        {stats.ordersByCity.every((c) => c.orders === 0) ? (
          <p className="text-sm text-muted-foreground">{t("لا توجد طلبات مسجلة بعد.", "No orders recorded yet.")}</p>
        ) : (
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={stats.ordersByCity}>
              <XAxis dataKey="city" tick={{ fontSize: 12 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
              <Tooltip />
              <Bar dataKey="orders" fill="#0F7654" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </section>
    </div>
  );
}
